...
@section('title', 'Seller Cart | Seller Ecom530')
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <div class="ibox">
                                    ...
                                    <td>
                                       <form class="form-validate" method="post" enctype="multipart/form-data" action="{{route('cart.update', $value->id)}}">
                                            ...
                                            <select class="status" name="status" onchange="this.form.submit()" class="form-control" required>
                                                ...
                                                <option value="new" {{$cart_data_avail && ( 'new'== $value->status) ? "selected = 'selected'" : ""}} disabled>
                                                    New
                                                </option>
                                                <option value="cancelled" {{$cart_data_avail && ( 'cancelled'== $value->status) ? "selected = 'selected'" : ""}} disabled>
                                                    Cancelled
                                                </option>
                                                <option value="verified" {{$cart_data_avail && ( 'verified'== $value->status) ? "selected = 'selected'" : ""}} disabled>
                                                    Verified
                                                </option>
                                                <option value="processing" {{$cart_data_avail && ( 'processing'== $value->status) ? "selected = 'selected'" : ""}} disabled>
                                                    Processing
                                                </option>      
